class JobsController < ApplicationController
  ...
  def index
    @jobs = case params[:order]
           ...
           when 'by_workplace'
             Job.published.order('workplace DESC').paginate(:page => params[:page], :per_page => 5)
           else
...
private
 ...
 def job_params
   params.require(:job).permit(:title, :description, :workplace, :wage_upper_bound, :wage_lower_bound, :contact_email, :website, :is_hidden)
 end
end
